...
_deploy: _docker-build-image _login-with-docker-registry
	$(info Deploying application $(APP))
	@ make _publish-docker-image APP=$(APP) VERSION=$$TRAVIS_BUILD_ID
	@ make _publish-docker-image APP=$(APP) VERSION=latest
	make _release APP=$(APP) IMAGE_ID=$$(docker image inspect registry.heroku.com/$(APP)/web:latest -f {{.Id}} )
...
_release:
	$(info Create relaese to $(APP) from image id $(IMAGE_ID))
	@ curl -X PATCH \
